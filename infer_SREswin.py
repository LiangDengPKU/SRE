...
model_t.load_state_dict(torch.load(model_path, map_location=device), strict=True)
model_t.eval()
...
def pre_img_batch(imgs_, n_):
    # imgs_: [N, H_in, W_in, 3] —— NHWC, uint8 or float
    resized_imgs = []
    for i in range(n_):
        img = imgs_[i]  # [H, W, 3]
        img_resized = cv2.resize(img, (INPUT_DIMENSION, INPUT_DIMENSION), interpolation=cv2.INTER_CUBIC)
        resized_imgs.append(img_resized)
    
    batch_images_tf_ = np.stack(resized_imgs, axis=0)  # [N, H_out, W_out, 3]
    batch_images_tf_ = np.transpose(batch_images_tf_, (0, 3, 1, 2))  # → [N, 3, H, W]
