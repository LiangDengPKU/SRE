    ...
    def forward(self, x):
        ...
        if H % self.patch_size[0] != 0 or W % self.patch_size[1] != 0:
            raise ValueError(
                ...
                f"patch size {self.patch_size[0]}*{self.patch_size[1]}."
            )
...
class BasicLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        super().__init__()
        #-------------------------------------------------------#
        #   4 stage dim [96, 192, 384, 768]
        #-------------------------------------------------------#
        self.dim                = dim
        #-------------------------------------------------------#
        #   4 stage input [[56, 56], [28, 28], [14, 14], [7, 7]]
        #-------------------------------------------------------#
        self.input_resolution   = input_resolution
        #-------------------------------------------------------#
        #   4 stage multihead [2, 2, 6, 2]
        #-------------------------------------------------------#
        self.depth              = depth
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x_, x = layer(x)
            if i != 0:
                interval_outs.append(x_)
        
        outs = []
        for i, layer in enumerate(interval_outs):
